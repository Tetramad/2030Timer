...
static inline void twii_enable(void) {
    TWCR = _BV(TWEN) | _BV(TWIE);
}

static inline void twii_bitrate(uint8_t bitrate) {
    TWBR = bitrate;
}

static inline void twii_prescaler(uint8_t prescaler) {
    TWSR = prescaler & 0b10 ? TWSR | _BV(TWPS1) : TWSR & ~_BV(TWPS1);
    TWSR = prescaler & 0b01 ? TWSR | _BV(TWPS1) : TWSR & ~_BV(TWPS1);
}

static inline void twii_write_data(uint8_t sla, uint8_t loc, uint8_t data) {
    while (twic != TWIC_STO) {}
    twic_sla = sla;
    twic_loc = loc;
    twic_data = data;
...
static inline void twii_read_data(uint8_t sla, uint8_t loc, uint8_t *receive) {
    while (twic != TWIC_STO) {}
    twic_sla = sla;
    twic_loc = loc;
